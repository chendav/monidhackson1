...
| QR-38 | Vercel S3 secrets/attestations are treated as deployed without a new build | Redeploy committed revision, store exact deployment-bound receipts, and repeat health/remote smoke before live mutations | pass for current deployment `dpl_md5xRevqZNJiDYG4Z6mtjWF4JCQd`; exact receipt stored and public smoke passed 4/4 |
| QR-39 | Provider code runs against a drifted Monid/OpenAI contract | Require a current provider-contract attestation before source or paid work and test mismatch/expiry paths | canonical Monid inspect hash/config pinned and two live parses passed; exact deployment/OpenAI receipt open |
...
| QR-42 | The integrated pre-deploy suite regresses after hardening | Run check, build, local E2E, official fixtures, production audit, and full high/critical gate | pass: check 44 files/421 tests with 4 files/10 tests skipped; build 8 steps/4 workflows/13 pages; E2E 14/2; fixtures 3/3; production audit clean; full audit 0 high/critical with 1 low/3 moderate development findings |
...
| QR-45 | A slow durable pre-dispatch write leaves OpenAI with a stale timeout computed before the paid call | Recompute the aggregate deadline immediately after the durable callback and prove an expired deadline sends zero paid requests | pass: focused regression records a zero-cost failed settlement with `attemptedBatches=0`; current full suite 421 passed/10 skipped |
...
| QR-47 | Context.dev retention is hidden or overstated | Verify the source form and audit view disclose unavailable ZDR and the observed seven-day artifact expiry; health exposes the same release fact | implemented pending full E2E/deploy verification |
...
| QR-49 | Cost UI treats unpriced infrastructure as zero | Require explicit partial completeness, known subtotal, unavailable providers, and not-applicable providers | implemented and focused unit/E2E coverage added; full gate pending |
...
| QR-51 | A `complete` cost panel omits billable Workflow, Queue, or database capacity dimensions | Emit and verify each active-provider SKU, bind Workflow retention to run TTL, live-attest the Neon CU ceiling, and fail closed when any required operation is absent | pass locally: 24 generated function invocations and the USD 1.412123 five-document full reserve are pinned; production capacity/usage receipt pending new deploy |
