...
| QR-38 | Vercel S3 secrets/attestations are treated as deployed without a new build | Redeploy committed revision, store exact deployment-bound receipts, and repeat health/remote smoke before live mutations | pass for captured deployment `dpl_EW9Bt6QLnhbMSwhEL5yY3AaJ64GE`; repeat after evidence-only deployment |
...
| QR-41 | Cleanup maintenance silently stops or runs against stale code | Require an authenticated bounded heartbeat and freshness gate | Railway scheduled delivery passed three cycles and independent review P0/P1/P2=0; GitHub remains unproven redundancy |
...
| QR-46 | Railway Cron becomes an always-on or over-privileged application service | Inspect digest, domains, variables-by-name, restart policy, instance lifecycle, logs, and three durable heartbeats | pass: no domain/volume, only maintenance URL/secret, restart NEVER, instances exited, three cycles, Reviewer APPROVE P0/P1/P2=0 |
