...
npm run evidence:check # fail closed on live placeholders or open evidence gates
npm run test:evidence  # deterministic evidence-gate unit tests
npm run build        # evidence gate + full HyperFrames check
...
`npm run build`, `npm run check`, `npm run render`, and `npm run publish` all
run the evidence gate first. Do not replace those canonical commands with a
direct `npx hyperframes render` or `publish`: that bypasses the live-evidence
guard. A checked item in `ASSET_GATES.md` must name a real project-relative
evidence file and its matching SHA-256.
